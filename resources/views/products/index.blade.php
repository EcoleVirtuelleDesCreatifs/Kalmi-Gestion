...
<head>
    ...
    <title>Produits - Kalmi Gestion</title>
    ...
    <div class="flex flex-col lg:flex-row min-h-screen">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <header class="lg:hidden bg-white shadow-sm px-4 py-3">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-bold text-gray-800">Produits</h2>
                    <a href="{{ route('profile.edit') }}" class="text-gray-600 hover:text-gray-900">
            ...
            <main class="p-3 sm:p-4 lg:p-6">
                <div class="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h2 class="text-xl lg:text-2xl font-bold text-gray-800">Produits</h2>
                        <p class="text-sm text-gray-600 mt-1">Gestion des produits et stocks</p>
                    </div>
                    <a href="{{ route('products.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition text-sm sm:text-base">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($products as $product)
                                        <tr class="{{ $product->stock_quantity <= $product->alert_threshold ? 'bg-red-50' : '' }} hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $product->name }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $product->category->name ?? '-' }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($product->purchase_price, 2) }} FCFA</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($product->selling_price, 2) }} FCFA</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                <span class="{{ $product->stock_quantity <= $product->alert_threshold ? 'text-red-600 font-bold' : 'text-gray-900' }}">
                                                    {{ $product->stock_quantity }}
                                                </span>
                                                @if($product->stock_quantity <= $product->alert_threshold)
                                                    <span class="text-xs text-red-600 ml-1"><i class="fas fa-exclamation-triangle"></i></span>
                                                @endif
                                            </td>
